...
const BlogPostClient = ({ post: initialPost }) => {
  ...
  // Fetch post if not provided from server
  useEffect(() => {
    if (initialPost) {
      setIsLoading(false);
      return;
  ...
  useEffect(() => {
    if ((error || (!isLoading && !data)) && !initialPost) {
    ...
    <main className="pt-[110px] min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="w-full flex lg:flex-row flex-col gap-8 my-10">
          {/* Article Content */}
          <article className="w-full lg:w-[70%] flex flex-col gap-5">
            <div
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:items-center mt-8 pt-6 border-t border-[#E1E6EB]">
              <p className="font-medium text-[#28305F] mb-2 sm:mb-0 whitespace-nowrap">
                Share this article:
              </p>
          ...
          <aside className="lg:w-[30%] w-full">
            <div className="flex flex-col md:gap-6 gap-6 lg:sticky lg:top-[100px]">
              <SearchSuggestion />

              {/* CTA Cards */}
              <div className="bg-[#41BBC8] p-6 flex flex-col gap-4 text-white rounded-lg">
                ...
                <Link
                  ...
                  className="bg-[#28305F] text-white font-medium text-[16px] p-3 text-center hover:bg-[#1e2347] transition-colors mt-2 rounded"
                >
                  Book at Beaches
                </Link>
              </div>

              <div className="bg-[#28305F] p-6 flex flex-col gap-4 text-white rounded-lg">
                <p className="font-semibold text-[20px] leading-[28px]">
                  See better with 360 Eyecare
                </p>
                <p className="text-[16px] font-normal leading-[24px]">
                  Providing complete Eye Exam & premium vision care in Toronto's
                  vibrant Yorkville neighbourhood.
                </p>
                <Link
                  href="/toronto-rosedale-optometrist"
                  className="bg-[#41BBC8] text-white font-medium text-[16px] p-3 text-center hover:bg-[#369ca8] transition-colors mt-2 rounded"
                >
                  Book at Yorkville
                </Link>
              </div>
